...
|---|---|---|---|---|
| 2026-09-07 | Every record written this session was dated 2026-09-05 — the previous session's date, carried forward without rechecking. The catch-log row above, the Gate 2 report's header, and the freshness marker were all wrong by two days in the files that exist to be an accurate record. The drift checker's freshness check caught it, alongside the two now-stale `#18` claims. | automatic-gate | artifact-vs-effect | dates corrected. Second true positive for that checker, and the first where it caught something about MY work rather than about a stale plan |
| 2026-09-07 | Two table rows in THIS file were concatenated onto one line — an appended row written without a leading newline swallowed the `vacuous-test` promotion summary into the end of the `destructive-scope` row. The promotion record was invisible when rendered and the table was unparseable at that line. Found while recounting the log for the state doc, not by any check. | agent-self | artifact-vs-effect | split into two rows. The log now parses to 37 rows with zero malformed ones; the recount is what surfaced it, which is an argument for counting the instrument rather than reading it |
...
| 2026-09-05 | — | — | `vacuous-test` | promoted -> verification.md catch **3b**, 3 rows (1 human, 2 agent-self), 2026-09-05. None found by a gate; each needed a deliberate mutation step. |
| 2026-09-05 | — | — | `silent-truncation` | promoted -> verification.md catch **8**, 3 rows (1 human, 2 agent-self), 2026-09-05 |
| 2026-09-05 | Promoted catches 3b and 8 into `docs/checklists/verification.md` — shipped plugin content — without bumping the plugin version. Consumers would have kept the cached copy and never received either check: the promotion would have been recorded here and invisible everywhere it was meant to apply. CI caught it on push. | automatic-gate | artifact-vs-effect | fixed — plugin 0.4.0 -> 0.5.0. Worth noting the shape: the change that gets forgotten is the DOCS half of a change whose code half needed no version bump |
| 2026-09-05 | The pre-commit exec-bit guard blocked a commit that would have shipped `check-memory-drift.sh` and its test as `100644`. Both had shebangs and both showed `-rwxr-xr-x` in the working tree over UNC — the index said otherwise, and the index is what ships. Without the guard, two scripts would have landed inert. | automatic-gate | artifact-vs-effect | fixed with `git update-index --chmod=+x`. Third time this guard has paid for itself; the working-tree mode over UNC remains untrustworthy and only the index view is |
| 2026-09-05 | The new drift checker found real drift on its first live run: `current-state.md` carried `Last updated: 2026-08-23` after being edited several times today. The marker is exactly the kind of claim that decays silently, since nothing else reads it. | automatic-gate | artifact-vs-effect | marker corrected to 2026-09-05; the run is now clean. The instrument found a true positive on the file it was built for, on its first use |
